...
// This function is now shared and can be imported by multiple dashboard pages.
export const mapRawApplicantData = (app: any): NewIntakeApplicationData => {
    ...
    } else if (!fullNameConstructed && app.fullName) { 
    ...
    return {
        ...
        department: app.department || undefined, // Added from User type, map if available
        level: app.level || undefined, // Added from User type, map if available
        ...
        experiences: (app.experiences || []).map((exp: any) => ({
            ...
            file: exp.document_file_name ? { name: exp.document_file_name, type: exp.document_file_type, size: parseInt(String(ol.certificate_file_size || 0), 10) } : (exp.file || null),
        })),
    };
};
