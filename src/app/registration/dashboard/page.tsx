...
import { CalendarIcon, PlusCircle, Trash2, UploadCloud, User as UserIcon, Loader2, ArrowRight, ArrowLeft, CheckCircle, XCircle, Hourglass, FileClock, UserCheck, Printer, RefreshCw, Check, MailWarning, UserCog, BookCopy, GraduationCap, HeartHandshake, Briefcase, FileClock as FileClockIcon } from "lucide-react";
...
const photographFileSchema = z.custom<FileList>()
...
const documentFileSchema = z.custom<FileList>()
...
export default function RegistrationDashboardPage() {
  ...
  const nextTab = async () => {
    const currentStepIndex = formTabs.findIndex(step => step.id === currentTab);
    const currentFields = formTabs[currentStepIndex].fields as FieldName[] | undefined;
    let output = true;
    ...
    // Step-specific custom checks
    if (currentStepIndex === 0) {
        // Custom check for photograph: Must have either an existing one or a new selection
        const hasExistingPhoto = !!form.getValues("photograph");
        const photoFiles = form.getValues("photographFile");
        const hasNewPhoto = !!photoFiles && photoFiles.length > 0;
        
        if (!hasExistingPhoto && !hasNewPhoto) {
            form.setError("photographFile", { type: "manual", message: "Passport photograph is required." });
            output = false;
        }
    }
    ...
    if (!output) {
        toast({
            ...
            description: "Please fill in all required fields correctly before moving to the next section."
        });
        return;
    }

    if (currentStepIndex < formTabs.length - 1) {
      setCurrentTab(formTabs[currentStepIndex + 1].id);
            ...
            <CardContent>
                {appStatus === "Not Submitted" && (
                    <div className="flex items-center p-4 bg-muted/50 rounded-md border-l-4 border-destructive">
                        <FileClockIcon className="h-8 w-8 text-destructive mr-4" />
                        <div>
                                ...
                                <Form {...form}>
                                    ...
                                    <TabsContent value="program" className="space-y-6">
                                        ...
                                        <FormField control={form.control} name="preferredProgram" render={({ field }) => (
                                            <FormItem><FormLabel>Preferred Program</FormLabel><Select onValueChange={field.onChange} defaultValue={field.value}>
                                        ...
                                        <FormField control={form.control} name="preferredCampus" render={({ field }) => (
                                            <FormItem><FormLabel>Preferred Campus</FormLabel><Select onValueChange={field.onChange} defaultValue={field.value}>
                                        ...
                                        <FormField control={form.control} name="entryMode" render={({ field }) => (
                                            <FormItem><FormLabel>Entry Mode</FormLabel><Select onValueChange={field.onChange} defaultValue={field.value}>
