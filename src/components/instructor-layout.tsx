...
import InstructorNavLinks from "@/components/instructor-nav-links";
    ...
    <SidebarProvider defaultOpen={true} open={true} onOpenChange={() => {}}>
      <Sidebar variant="inset" side="left" collapsible="icon">
        <SidebarHeader className="p-4 items-center">
          <Link href="/instructor/dashboard" className="flex items-center gap-2 group-data-[collapsible=icon]:justify-center">
            <ArewaLogo className="h-8 w-8 text-sidebar-primary" />
            <span className="font-semibold text-lg text-sidebar-primary group-data-[collapsible=icon]:hidden">Instructor Portal</span>
          </Link>
        </SidebarHeader>
      ...
      <SidebarInset>
        ...
        <footer className="border-t p-4 text-center text-xs text-muted-foreground">
          &copy; {new Date().getFullYear()} SIAT-Institute, Zaria - Instructor Portal.
        </footer>
      </SidebarInset>
    </SidebarProvider>
  );
